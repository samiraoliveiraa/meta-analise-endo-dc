...
qiime dada2 denoise-paired \
  --i-demultiplexed-seqs "temp/demux_${DIR_NAME}-trimmed.qza" \
  --p-trim-left-f 20 \
  --p-trunc-len-f 145 \
  --p-trim-left-r 20 \
  --p-trunc-len-r 135 \
...
qiime feature-classifier classify-sklearn \
  --i-classifier /home/public/microbiota_jubs_sams/03-qiime/silva-data/silva-138.1-v4-classifier.qza \
...
qiime picrust2 full-pipeline \
   --i-table "temp/table_${DIR_NAME}.qza" \
   --i-seq "temp/rep-seqs_${DIR_NAME}.qza" \
   --output-dir picrust2 \
...
qiime metnet differentialReactions \
  ...
  --p-condition-name cpp \
  --p-control-name controle \
  --p-selection-model AGREDA \
  --o-differential-analysis "temp/reactions_differential_cpp.qza"

qiime metnet differentialReactions \
  --i-reactions "temp/output_reactions.qza" \
  --m-metadata-file "${DIR_NAME}_metadata.tsv" \
  --m-metadata-column group \
  --p-condition-name cpp_endo \
  --p-control-name controle \
  --p-selection-model AGREDA \
  --o-differential-analysis "temp/reactions_differential_cppendo.qza"
...
qiime metnet differentialSubSystems \
  ...
  --p-condition-name cpp \
  --p-control-name controle \
  --o-differential-analysis "temp/subsystems_differential_cpp.qza"
...
qiime metnet differentialSubSystems \
  ...
  --p-condition-name cpp_endo \
  --p-control-name controle \
  --o-differential-analysis "temp/subsystems_differential_cppendo.qza"

qiime metadata tabulate \
  --m-input-file "temp/subsystems_differential_cpp.qza" \
  --o-visualization "subsystems_${DIR_NAME}_cpp.qzv"

qiime metadata tabulate \
  --m-input-file "temp/subsystems_differential_cppendo.qza" \
  --o-visualization "subsystems_${DIR_NAME}_cppendo.qzv"
