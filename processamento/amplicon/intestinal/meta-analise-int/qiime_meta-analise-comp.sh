...
output_path="/home/public/microbiota_jubs_sams/03-qiime/intestinal/meta-analise-int/temp"
...
qiime diversity core-metrics \
  ...
  --m-metadata-file meta-analise-int_metadata.tsv \
...
mv pasta_temp/distance-matrix.tsv ../bc_meta-analise-int.tsv
...
qiime diversity beta-group-significance \
  ...
  --p-method permanova \
  --o-visualization ../beta_meta-analise-int.qzv

qiime diversity alpha-group-significance \
  --i-alpha-diversity shannon_vector.qza \
  --m-metadata-file ../meta-analise-int_metadata.tsv \
  --o-visualization ../shannon_meta-analise-int.qzv
...
qiime taxa barplot \
  ...
  --m-metadata-file "../meta-analise-int_metadata.tsv" \
  --o-visualization "ko_barplot_meta-analise-int.qzv"

qiime taxa barplot \
  --i-table "ec_metagenome.qza" \
  --m-metadata-file "../meta-analise-int_metadata.tsv" \
  --o-visualization "ec_barplot_meta-analise-int.qzv"
...
biom convert \
  ...
  -o "../pathway_meta-analise-int.tsv" \
  --to-tsv
...
qiime feature-table transpose \
  --i-table "temp/abundancia_unificada.qza" \
  --o-transposed-feature-table "temp/table_meta-analise-int_trans.qza"

qiime metnet generateFeatures \
  --i-frequency "temp/table_meta-analise-int_trans.qza" \
...
qiime metnet differentialReactions \
  ...
  --m-metadata-file "meta-analise-int_metadata.tsv" \
  --m-metadata-column group \
  --p-condition-name "Fibromialgia" \
  --p-control-name "Controle" \
  --p-selection-model AGREDA \
  --o-differential-analysis "temp/reactions_differential_fbm.qza"

qiime metnet differentialReactions \
  --i-reactions "temp/output_reactions.qza" \
  --m-metadata-file "meta-analise-int_metadata.tsv" \
  --m-metadata-column group \
  --p-condition-name "SII" \
  --p-control-name "Controle" \
  --p-selection-model AGREDA \
  --o-differential-analysis "temp/reactions_differential_sii.qza"

qiime metnet differentialReactions \
  --i-reactions "temp/output_reactions.qza" \
  --m-metadata-file "meta-analise-int_metadata.tsv" \
  --m-metadata-column group \
  --p-condition-name "SFC" \
  --p-control-name "Controle" \
  --p-selection-model AGREDA \
  --o-differential-analysis "temp/reactions_differential_sfc.qza"
...
qiime metnet differentialSubSystems \
  ...
  --p-condition-name "Fibromialgia" \
  --p-control-name "Controle" \
  --o-differential-analysis "temp/subsystems_differential_fbm.qza"
...
qiime metnet differentialSubSystems \
  ...
  --p-condition-name "SII" \
  --p-control-name "Controle" \
  --o-differential-analysis "temp/subsystems_differential_sii.qza"
...
qiime metnet differentialSubSystems \
  ...
  --p-condition-name "SFC" \
  --p-control-name "Controle" \
  --o-differential-analysis "temp/subsystems_differential_sfc.qza"



qiime metadata tabulate \
  --m-input-file "temp/subsystems_differential_endo.qza" \
  --o-visualization "subsystems_meta-analise-int_endo.qzv"

qiime metadata tabulate \
  --m-input-file "temp/subsystems_differential_fbm.qza" \
  --o-visualization "subsystems_meta-analise-int_fbm.qzv"

qiime metadata tabulate \
  --m-input-file "temp/subsystems_differential_sii.qza" \
  --o-visualization "subsystems_meta-analise-int_sii.qzv"

qiime metadata tabulate \
  --m-input-file "temp/subsystems_differential_sfc.qza" \
  --o-visualization "subsystems_meta-analise-int_sfc.qzv"
