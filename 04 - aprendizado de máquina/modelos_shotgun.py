...
os.makedirs("modelos-shotgun", exist_ok=True)
...
def otimizar_modelo(modelo_nome, X, y):
    ...
    def objetivo(trial):
        if modelo_nome == "LogReg":
            ...
            raise ValueError(f"Modelo '{modelo_nome}' nÃ£o suportado.")
        
        cv = KFold(n_splits=N_SPLITS, shuffle=True, random_state=SEMENTE_ALEATORIA)
    ...
    print(f"Melhor acurácia média (CV): {study.best_value:.3f}")
...
def avaliar_modelos(nome, dados):
    ...
    print(f'Número de táxons pré-filtro: {len(dados.columns)}')

    y = (dados["group"] != "controle").astype(int).values
    ...
    print(f'Número de táxons pós-Mann-Whitney + FDR (30%): {len(idx_top)}')
    ...
    for nome_modelo in modelos:
        ...
        if nome_modelo == "LogReg":
            modelo = LogisticRegression(max_iter=5000, solver='saga', random_state=SEMENTE_ALEATORIA, **best_params)
        elif nome_modelo == "RandomForest":
            modelo = RandomForestClassifier(random_state=SEMENTE_ALEATORIA, **best_params)
        ...
        caminho_modelo = f"modelos-shotgun/{nome_limpo}_{nome_modelo}.joblib"
